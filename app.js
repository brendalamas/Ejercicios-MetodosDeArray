...
const numeroMasDiez = numeros.map((numero)=>{
    const sumando = (numero+10)
    return sumando
})
...
const dobles = numerosAlAzar.map((numero)=>{
    const multiplicacion = numero*2
    return multiplicacion
})

console.log(numerosAlAzar); // [3, 7, 13, 99]
...
const nuevoArray = librosDeJS.map((libro)=>{
    return `<li>${libro}</li>`
})
...
const longitudes = frases.map((frase, index, array)=>{
    return frase.length
})
...
const playlistConIndex = playlist.map((play, index, array)=>{
    return `${index} - ${play}`
})

console.log(playlistConIndex);
// RESULTADO ESPERADO
...
const preciosFinales = costos.map((costo)=>{
    const resultadoParcial = sumarGanancia(costo)
    const resultadoTOTAL = agregarIVA(resultadoParcial)
...
const costosSegunProducto = [ 12.5, 56, 98 ];
...
const preciosFinalesCostoYProducto = costosSegunProducto.map((costo)=>{
    const resultadoPARCIAL = sumarGanancia(costo)
    const resultadoFINAL = agregarIVA(resultadoPARCIAL)
    return `${productos[0]} ${resultadoFINAL}`
})

console.log(preciosFinalesCostoYProducto);
// deberia mostrar
...
const costosFilter = [ 39, 53, 17, 99, 7, 9, 6, 68, 54, 97, 27, 90, 92, 75, 26, 86, 22, 42, 20, 14 ];
// codear acá la solución del ejercicio
const losMasCaros = costosFilter.filter((costo)=>{
    return costo > 50
})

console.log(costosFilter);
...
const buscandoParOImpar = masNumeros.filter((elemento,index, array)=>{
    if(elemento % 2 === 0){
...
// codear acá la solución del ejercicio
const soloStrings = mix.filter ((elemento, index, array)=>{
...
const playlistFilter = ['Smells Like Teen Spirit', 'Everlong', 'Come As You Are', 'The Pretender', 'Heart-Shaped Box', 'Learn to Fly', 'Lithium'];
var playlistEscuchada = ['The Pretender', 'Lithium', 'Come As You Are']

// codea aca tu solución
const playlistSinEscuchar = playlistFilter.filter((elemento, index, array)=>{
    
    return playlist.includes(`${playlistEscuchada}`)
})
